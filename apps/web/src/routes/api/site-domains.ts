...
export const ServerRoute = createServerFileRoute("/api/site-domains").methods({
  GET: async ({ request }) => {
    ...
    if (!orgId || !siteId) {
      return new Response(JSON.stringify({ error: "Missing org or site" }), {
        status: 400,
        headers: { "content-type": "application/json" },
      })
    }
    ...
    originUrl.searchParams.set("table", "site_domains")
